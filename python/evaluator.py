...
class Evaluator:
    def __init__(self, expressions: list[Expression], output_area: tk.Label, error_flag = False):
    ...
    def process(self):
        try:
            for expression in self.expressions:
                ...
                if not result.is_ok:
                    raise TypeError(f'FATAL ERROR: {result.error.message}')
                    error_flag = True
                    return
        except Exception as e:
            raise TypeError(f'FATAL ERROR: {e}')
    ...
    def process_print(self, expression: Expression):
        ...
        for argument in expression.operands:
            ...
            result = str(value_data.value)

        text = self.output_area["text"] + "\n"
        self.output_area.config(text=text+result)
